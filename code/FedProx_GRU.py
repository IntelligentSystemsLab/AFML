...
class FedProx_GRU(nn.Module):
  def __init__(self, city,device,num,):
    super(FedProx_GRU, self).__init__()
    self.device = device
    self.city = city
    ...
    self.writer = SummaryWriter('./results/{}/l_{}_s_{}_b_{}_i_{}_n_{}/FedProx_GRU'.format(self.city,LEN_INPUT,TEST_UPDATE_STEP,BATCH_SIZE,BATCH_INDEX,self.number))
  ...
  def Fed_Train(self,round):
    ...
    for id,j in enumerate(id_train_0):
        ...
        self.clients[j].Local_FedProx_Train()
