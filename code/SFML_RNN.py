...
class SFML_RNN(nn.Module):
  def __init__(self, city,device,num,):
    ...
    self.writer = SummaryWriter('d:/cqy/charge_station_tits/results/{}/l_{}_s_{}_b_{}_i_{}_n_{}/SFML_RNN'.format(self.city,LEN_INPUT,TEST_UPDATE_STEP,BATCH_SIZE,BATCH_INDEX,self.number))

    for index,path in enumerate(train_path_set):
      model = RNNNet_BN().to(self.device)
