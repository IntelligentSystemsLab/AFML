...
import torch
import os
from PerFedAvg_LSTM import PerFedAvg_LSTM
...
def main(model_name,device_num,epoch,num):
    folder = r"d:/cqy/charge_station_tits/model/{}".format(CITY)
    if not os.path.exists(folder):
        os.mkdir(folder)
    folder = r"d:/cqy/charge_station_tits/model/{}/l_{}_s_{}_b_{}_i_{}_n_{}".format(CITY,LEN_INPUT,TEST_UPDATE_STEP,BATCH_SIZE,BATCH_INDEX,num)
    if not os.path.exists(folder):
        os.mkdir(folder)
    folder = os.path.join(folder,model_name)
    if not os.path.exists(folder):
        os.mkdir(folder)
    fed_net = PerFedAvg_LSTM(CITY,device_num,num)
