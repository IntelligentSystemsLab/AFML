...
class FedAsync_GRU(nn.Module):
  def __init__(self, city,device,num,):
    super(FedAsync_GRU, self).__init__()
    self.device = device
    self.city = city
    ...
    self.writer = SummaryWriter('./results/{}/l_{}_s_{}_b_{}_i_{}_n_{}/FedAsync_GRU'.format(self.city,LEN_INPUT,TEST_UPDATE_STEP,BATCH_SIZE,BATCH_INDEX,self.number))
