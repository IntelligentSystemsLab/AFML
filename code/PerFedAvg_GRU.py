...
class PerFedAvg_GRU(nn.Module):
  def __init__(self, city,device,num,):
    super(PerFedAvg_GRU, self).__init__()
    self.device = device
    self.city = city
    self.time_len = LEN_INPUT
    self.number = num
    self.deep_frequency = REFRESH_FREQUENCY
    self.net = GRUNet().to(self.device)
    train_path = r"d:/cqy/charge_station_tits/dataset/{}_save/train".format(self.city)
    test_path = r"d:/cqy/charge_station_tits/dataset/{}_save/test".format(self.city)
    ...
    self.clients = []
    self.test_clients = []
    self.mode_1 = "PerFedAvg_train"
    self.mode_2 = "PerFedAvg_test"
    self.writer = SummaryWriter('d:/cqy/charge_station_tits/results/{}/l_{}_s_{}_b_{}_i_{}_n_{}/PerFedAvg_GRU'.format(self.city,LEN_INPUT,TEST_UPDATE_STEP,BATCH_SIZE,BATCH_INDEX,self.number))

    for index,path in enumerate(train_path_set):
      model = GRUNet().to(self.device)
  ...
  def Fed_Train(self,round):
    ...
    for id,j in enumerate(id_train_0):
        ...
        self.clients[j].Local_Fomaml_Train()
